...
This project provides a Docker container for [Firefox ESR](https://www.mozilla.org/en-CA/firefox/enterprise/).
...
|`FF_OPEN_URL`| The URL to open when Firefox ESR starts. Multiple URLs can be opened by separating them with the pipe character (`|`). | (no value) |
...
|`FF_CUSTOM_ARGS`| Custom argument(s) to pass when launching Firefox ESR. | (no value) |
...
services:
  firefox-esr:
    image: jlesage/firefox-esr
    ports:
      - "5800:5800"
    volumes:
      - "/docker/appdata/firefox-esr:/config:rw"
```
...
[Releases]: https://github.com/jlesage/docker-firefox-esr/releases
[Docker Hub]: https://hub.docker.com/r/jlesage/firefox-esr/tags
...
```shell
docker pull jlesage/firefox-esr
```
...
container, it would proxy all HTTP requests for `firefox-esr.domain.tld` to
...
server {
	...
	server_name firefox-esr.domain.tld;

	location / {
	        proxy_pass http://docker-firefox-esr;
	}
...
server {
	...
	location = /firefox-esr {return 301 $scheme://$http_host/firefox-esr/;}
	location /firefox-esr/ {
		proxy_pass http://docker-firefox-esr/;
		# Uncomment the following line if your Nginx server runs on a port that
		# differs from the one seen by external clients.
		#port_in_redirect off;
		location /firefox-esr/websockify {
			proxy_pass http://docker-firefox-esr/websockify;
			proxy_http_version 1.1;
			proxy_set_header Upgrade $http_upgrade;
			proxy_set_header Connection $connection_upgrade;
			proxy_read_timeout 86400;
		...
		# Needed when audio support is enabled.
		location /firefox-esr/websockify-audio {
			proxy_pass http://docker-firefox-esr/websockify-audio;
			proxy_http_version 1.1;
			proxy_set_header Upgrade $http_upgrade;
			proxy_set_header Connection $connection_upgrade;
			proxy_read_timeout 86400;
...
| **Application version** label | Displays the version of Firefox ESR integrated into Docker image. |
...
Any desktop notification generated by Firefox ESR is forwarded to the
...
To properly work, recent versions of Firefox ESR need the
...
For Firefox ESR to be able to use the audio device available on
the host, `/dev/snd` must be exposed to the container by adding the
`--device /dev/snd` parameter to the `docker run` command.

## Setting Firefox ESR Preferences Via Environment Variables

Firefox ESR preferences can be set via environment variables
...
via Firefox ESR directly.

## Troubleshooting

### Crashes

If Firefox ESR is crashing frequently, make sure that:
